...
-- If there are more than one person with the same number of oscar wins, return 
-- the first one in alphabetic order based on their name. Use the names as keys when joining the tables.
...
WITH CTE AS(
  SELECT COUNT(winner) AS'count_awards',
  nominee
  FROM oscar_nominees
  WHERE winner = 1
  GROUP BY nominee
)
SELECT n.top_genre, CTE.count_awards
FROM nominee_information n 
JOIN CTE
ON n.name = CTE.nominee 
ORDER BY CTE.count_awards DESC, n.name
LIMIT 1;
